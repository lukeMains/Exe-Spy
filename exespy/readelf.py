...
def _get_cu_base(cu):
    ...
    if "DW_AT_low_pc" in attr:
        ...
        rl = cu.dwarfinfo.range_lists().get_range_list_at_offset(
            attr["DW_AT_ranges"].value, cu
        )
        base_ip = None
        for r in rl:
...
class ReadElf:
    ...
    def display_file_header(self):
        ...
        self._emitline(
            "  ABI Version:                       %d" % e_ident["EI_ABIVERSION"]
        )
        self._emitline(
            "  Type:                              %s"
        ...
        self._emitline(
            "  Size of this header:               %s (bytes)" % header["e_ehsize"]
        )
        self._emitline(
            "  Size of program headers:           %s (bytes)" % header["e_phentsize"]
        )
        self._emitline("  Number of program headers:         %s" % header["e_phnum"])
        self._emitline(
            "  Size of section headers:           %s (bytes)" % header["e_shentsize"]
        )
    ...
    def decode_flags(self, flags):
        ...
        if self.elffile["e_machine"] == "EM_ARM":
            ...
            if (flags & E_FLAGS.EF_RISCV_FLOAT_ABI) == E_FLAGS.EF_RISCV_FLOAT_ABI_SOFT:
                description += ", soft-float ABI"
            if (
                flags & E_FLAGS.EF_RISCV_FLOAT_ABI
            ...
            if (
                flags & E_FLAGS.EF_LOONGARCH_ABI_MODIFIER_MASK
            ) == E_FLAGS.EF_LOONGARCH_ABI_SOFT_FLOAT:
                description += ", SOFT-FLOAT"
            if (
                flags & E_FLAGS.EF_LOONGARCH_ABI_MODIFIER_MASK
            ) == E_FLAGS.EF_LOONGARCH_ABI_SINGLE_FLOAT:
                description += ", SINGLE-FLOAT"
            if (
                flags & E_FLAGS.EF_LOONGARCH_ABI_MODIFIER_MASK
            ) == E_FLAGS.EF_LOONGARCH_ABI_DOUBLE_FLOAT:
                description += ", DOUBLE-FLOAT"
            if (
                flags & E_FLAGS.EF_LOONGARCH_OBJABI_MASK
            ) == E_FLAGS.EF_LOONGARCH_OBJABI_V0:
                description += ", OBJ-v0"
            if (
                flags & E_FLAGS.EF_LOONGARCH_OBJABI_MASK
            ) == E_FLAGS.EF_LOONGARCH_OBJABI_V1:
                description += ", OBJ-v1"
            ...
            if self.elffile.elfclass == 32:
                self._emitline(
                    ...
                    % (
                        ...
                        self._format_hex(
                            section["sh_entsize"], fieldsize=2, lead0x=False
                        ),
                ...
                self._emitline(
                    ...
                    % (
                        ...
                        self._format_hex(
                            section["sh_entsize"], fullhex=True, lead0x=False
                        ),
                        describe_sh_flags(section["sh_flags"]),
                        section["sh_link"],
                        section["sh_info"],
    ...
    def display_symbol_tables(self):
        ...
        for section_index, section in symbol_tables:
            ...
            for nsym, symbol in enumerate(section.iter_symbols()):
                ...
                if (
                    symbol["st_info"]["type"] == "STT_SECTION"
        ...
        for section in self.elffile.iter_sections():
            ...
            for rel in section.iter_relocations():
                ...
                if rel["r_info_sym"] == 0:
                    ...
                    # readelf.
                    if symbol["st_name"] == 0:
                        symsecidx = self._get_symbol_shndx(
                            symbol, rel["r_info_sym"], section["sh_link"]
                        )
                ...
                # binaries.
                if (
                    self.elffile.elfclass == 64
                    and self.elffile["e_machine"] == "EM_MIPS"
                ):
                    for i in (2, 3):
                        rtype = rel["r_info_type%s" % i]
                        self._emit(
                            "                    Type%s: %s"
                            % (i, describe_reloc_type(rtype, self.elffile))
                        )
                        self._emitline()
            ...
            if isinstance(section, GNUVerSymSection):
                ...
                for idx_by_4 in range(0, num_symbols, 4):
                    ...
                    for idx in range(idx_by_4, min(idx_by_4 + 4, num_symbols)):
                        ...
                        self._emit(
                            "%4x%s%-13s" % (version_index, visibility, version_name)
                        )
                ...
                for verdef, verdaux_iter in section.iter_versions():
                    ...
                    self._emitline(
                        ...
                        "  Cnt: %i  Name: %s"
                        % (
                            self._format_hex(offset, fieldsize=6, alternate=True),
                            verdef["vd_version"],
                            flags,
                            verdef["vd_ndx"],
                            verdef["vd_cnt"],
    ...
    def display_hex_dump(self, section_spec):
        ...
        if section["sh_type"] == "SHT_NOBITS" or not (data := section.data()):
            self._emitline("Section '%s' has no data to dump." % (section_spec))
            return
    ...
    def _format_hex(
        self, addr, fieldsize=None, fullhex=False, lead0x=True, alternate=False
    ):
    ...
    def _print_version_section_header(
        self, version_section, name, lead0x=True, indent=1
    ):
    ...
    def _dump_debug_info(self):
        ...
        for cu in self._dwarfinfo.iter_CUs():
            ...
            self._emitline(
                "   Length:        %s (%s)"
                % (self._format_hex(cu["unit_length"]), "%s-bit" % cu.dwarf_format())
            )
            ...
            for die in cu.iter_DIEs():
                ...
                for attr in die.attributes.values():
                    ...
                    self._emitline(
                        "    <%x>   %-18s: %s%s"
                        % (attr.offset, name, attr_desc, postfix)
                    )
    ...
    def _dump_debug_types(self):
        ...
        for tu in self._dwarfinfo.iter_TUs():
            ...
            self._emitline(
                "   Length:        %s (%s)"
                % (self._format_hex(tu["unit_length"]), "%s-bit" % tu.dwarf_format())
            )
            self._emitline("   Version:       %s" % tu["version"])
            self._emitline(
                "   Abbrev Offset: %s"
                % (self._format_hex(tu["debug_abbrev_offset"], alternate=True))
            )
            ...
            for die in tu.iter_DIEs():
                ...
                for attr in die.attributes.values():
                    ...
                    self._emitline(
                        "    <%x>   %-18s: %s" % (attr.offset, name, attr_desc)
                    )
    ...
    def _dump_debug_line_programs(self):
        ...
        if not self._dwarfinfo.has_debug_info:
            return
        self._emitline(
            "Contents of the %s section:" % self._dwarfinfo.debug_line_sec.name
        )
        self._emitline()
        lineprogram_list = []
        ...
        for cu in self._dwarfinfo.iter_CUs():
            ...
            for entry in lineprogram.get_entries():
                ...
                if state is None:
                    ...
                    lineprogram["version"] < 4
                    or self.elffile["e_machine"] == "EM_PPC64"
                ):
                    self._emitline(
                        "%-35s  %11s  %18s    %s"
                        % (
    ...
    def _dump_frames_info(self, section, cfi_entries):
        ...
        for entry in cfi_entries:
            if isinstance(entry, CIE):
                ...
                self._emitline(
                    '  Augmentation:          "%s"' % bytes2str(entry["augmentation"])
                )
                if entry["version"] >= 4:
                    self._emitline(
                        "  Pointer Size:          %d" % entry["address_size"]
                    )
                    self._emitline(
                        "  Segment Size:          %d" % entry["segment_size"]
                    )
                self._emitline(
                    "  Code alignment factor: %u" % entry["code_alignment_factor"]
                )
                self._emitline(
                    "  Data alignment factor: %d" % entry["data_alignment_factor"]
                )
                self._emitline(
                    "  Return address column: %d" % entry["return_address_register"]
                )
                if entry.augmentation_bytes:
                    self._emitline(
                        "  Augmentation data:     {}".format(
                            " ".join(
                                "{:02x}".format(ord(b))
                ...
                length = (
                    entry["length"]
                    if entry.cie.offset < entry.offset
                    else entry.cie["length"]
                )
                self._emitline(
                    ...
                    % (
                        ...
                        self._format_hex(
                            entry["CIE_pointer"], fieldsize=8, lead0x=False
                        ),
                        entry.cie.offset,
                        self._format_hex(
                            entry["initial_location"], fullhex=True, lead0x=False
                        ),
                        self._format_hex(
    ...
    def _dump_debug_frames(self):
        ...
        if self._dwarfinfo.has_EH_CFI():
            self._dump_frames_info(
                self._dwarfinfo.eh_frame_sec, self._dwarfinfo.EH_CFI_entries()
            )
        self._emitline()

        if self._dwarfinfo.has_CFI():
            self._dump_frames_info(
                self._dwarfinfo.debug_frame_sec, self._dwarfinfo.CFI_entries()
            )
    ...
    def _dump_debug_namelut(self, what):
        ...
        for cu_hdr, (cu_ofs, items) in zip(
            cu_headers, itertools.groupby(namelut.items(), key=lambda x: x[1].cu_ofs)
        ):
            self._emitline(
                "  Length:                              %d" % cu_hdr.unit_length
            )
            self._emitline("  Version:                             %d" % cu_hdr.version)
            self._emitline(
                "  Offset into .debug_info section:     0x%x" % cu_hdr.debug_info_offset
            )
            self._emitline(
                "  Size of area in .debug_info section: %d" % cu_hdr.debug_info_length
            )
            self._emitline()
            self._emitline("    Offset  Name")
            for item in items:
                self._emitline(
                    "    %x          %s" % (item[1].die_ofs - cu_ofs, item[0])
                )
        self._emitline()
    ...
    def _dump_debug_aranges(self):
        ...
        self._emitline(
            "Contents of the %s section:" % self._dwarfinfo.debug_aranges_sec.name
        )
        self._emitline()
        prev_offset = None
        ...
        for cu in di.iter_CUs():
            for die in cu.iter_DIEs():
                for key in die.attributes:
                    ...
                    if LocationParser.attribute_has_location(
                        attr, cu["version"]
                    ) and LocationParser._attribute_has_loc_list(attr, cu["version"]):
    ...
    def _dump_loclist(self, loc_list, line_template, cu_map):
        ...
        for entry in loc_list:
            if isinstance(entry, LocationViewPair):
                ...
                self._emitline(
                    "    %08x v%015x v%015x location view pair"
                    % (entry.entry_offset, entry.begin, entry.end)
                )
            else:
                if in_views:
                    in_views = False
                ...
                if isinstance(entry, LocationEntry):
                    ...
                    begin_offset = (
                        0 if entry.is_absolute else base_ip
                    ) + entry.begin_offset
                    end_offset = (
                        0 if entry.is_absolute else base_ip
                    ) + entry.end_offset
                    ...
                    if has_views:
                        ...
                        postfix = (
                            " (start == end)"
                            if entry.begin_offset == entry.end_offset
                            and view.begin == view.end
                            else ""
                        )
                        self._emitline(
                        ...
                        self._emitline(
                            "             %016x %016x %s%s"
                            % (begin_offset, end_offset, expr, postfix)
                        )
                        loc_entry_count += 1
                    else:
                        postfix = (
                    ...
                    self._emitline(
                        "    %08x %016x (base address)"
                        % (entry.entry_offset, entry.base_address)
                    )
        ...
        last = loc_list[-1]
        self._emitline(
            "    %08x <End of list>" % (last.entry_offset + last.entry_length)
        )

    def _dump_debug_loclists_CU_header(self, cu):
        # Header slightly different from that of v5 rangelist in-section CU header dump
        self._emitline(
            "Table at Offset %s" % self._format_hex(cu.cu_offset, alternate=True)
        )
        self._emitline(
            "  Length:          %s" % self._format_hex(cu.unit_length, alternate=True)
        )
    ...
    def _dump_debug_rangesection(self, di, range_lists_sec):
        ...
        base_template_indexed = (
            "    %%08x %%0%dx (base address index) %%0%dx (base address)"
        ...
        cu_map = {
            die.attributes["DW_AT_ranges"].value: cu  # Range list offset => CU
            for cu in di.iter_CUs()
            for die in cu.iter_DIEs()
    ...
    def _dump_rangelist(
        ...
        for entry in range_list:
            if isinstance(entry, RangeEntry):
                postfix = (
                    " (start == end)" if entry.begin_offset == entry.end_offset else ""
                )
                self._emitline(
                    line_template
                    % (
                ...
                if ver5:
                    if not raw_v5_rangelist:
                        raw_v5_rangelist = range_lists_sec.get_range_list_at_offset_ex(
                            range_list[0].entry_offset
                        )
                    raw_v5_entry = next(
                        re
                        for re in raw_v5_rangelist
                        if re.entry_offset == entry.entry_offset
                    )
                if raw_v5_entry and raw_v5_entry.entry_type == "DW_RLE_base_addressx":
                    self._emitline(
                        base_template_indexed
                        % (entry.entry_offset, raw_v5_entry.index, entry.base_address)
                    )
                else:
                    self._emitline(
                        base_template
        ...
        self._emitline(
            "    %08x <End of list>"
            % (last.entry_offset + last.entry_length if ver5 else first.entry_offset)
        )
    ...
    def _display_attributes(self, attr_sec, descriptor):
        ...
        for s in attr_sec.iter_subsections():
            ...
            for ss in s.iter_subsubsections():
                h_val = (
                    ""
                    if ss.header.extra is None
                    else " ".join("%d" % x for x in ss.header.extra)
                )
...
def main(stream=None):
    ...
    argparser.add_argument(
        ...
        action="store_true",
        dest="show_traceback",
        help="Dump the Python traceback on ELFError exceptions from elftools",
    )
    ...
    with open(args.file, "rb") as file:
        try:
            ...
            if do_section_header:
                readelf.display_section_headers(show_heading=not do_file_header)
            if do_program_header:
                readelf.display_program_headers(show_heading=not do_file_header)
